...
(function () {
  ...
  const APP_VERSION = "0.68.0"; // bump with each shipped change so it's visible in Settings
  ...
  function buildCatFilter() {
    ...
    cats.forEach((c) => {
      ...
      edit.onclick = (ev) => { ev.stopPropagation(); finance ? Finance.openFinanceCatModal(c) : openCategoryModal(c); };
    ...
    addChip.onclick = (ev) => {
      ev.stopPropagation();
      finance ? Finance.openFinanceCatModal(null) : openCategoryModal(null);
    };
  ...
  function wire() {
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        ...
        Finance.closeFinanceModal(); Finance.cancelFinanceCatModal(); closeSettings();
        $("#addMenu").hidden = true;
      }
    });
  ...
  // Hand the finance module the shared app plumbing it renders/saves through.
  // Must run before init() — normalize() and the views call into Finance.
  Finance.init({
